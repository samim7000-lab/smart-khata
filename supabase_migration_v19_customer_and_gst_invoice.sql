...
-- 2. RECONCILE SHOPS TABLE (GST ENABLED, LEGAL NAME, GST REGISTRATION TYPE, STATE CODE, INVOICE SERIES)
...
-- Add check constraint for gst_registration_type if not exists
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Add check constraint for default_tax_mode if not exists
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Concurrency-Safe: Uses INSERT ... ON CONFLICT DO UPDATE to guarantee strict sequential numbers
-- Even if two merchant devices click Save at the exact same millisecond, they get unique consecutive numbers.
...
BEGIN
    ...
    -- Format short financial year (e.g. '2026-27' -> '26-27')
    ...
    -- Format standard consecutive invoice number e.g. INV/26-27/000001
...
-- 6. RECONCILE TRANSACTIONS TABLE (GST FIELDS, RECEIPT DETAILS & INVOICE REFERENCE)
...
-- 7. NOTIFY POSTGREST TO REFRESH SCHEMA CACHE
